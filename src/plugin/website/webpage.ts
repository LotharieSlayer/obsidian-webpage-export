...
.mod-header, .mod-footer, .metadata-container, .frontmatter, img[src^="data:"]`).forEach((heading) => heading.remove());
	...
	/* convert non-media file embeds (span.internal-embed.file-embed) into actual clickable
	 * download buttons. This must run AFTER remapEmbedLinks so the span's src has already been
	 * remapped to the correct exported attachment path (e.g. _resources/.../file.pur). If it ran
	 * any earlier, the href would be treated as a page link and rewritten into an .html page.
	 * The title is kept as a .file-embed-title child (with its icon) just like Obsidian, so the
	 * icon renders immediately to the left of the file name.
	 */
	private remapFileEmbeds() {
		const embeds = Array.from(this.pageDocument.querySelectorAll(".obsidian-document span.internal-embed.file-embed"));
		for (const embed of embeds) {
			const src = embed.getAttribute("src");
			if (!src) continue;

			const titleEl = embed.querySelector(".file-embed-title") as HTMLElement | null;
			const iconEl = titleEl?.querySelector(".file-embed-icon") as HTMLElement | null;
			const titleText = (titleEl?.childNodes
				? Array.from(titleEl.childNodes).filter((n) => n.nodeType == 3).map((n) => n.textContent ?? "").join("").trim()
				: "") || embed.getAttribute("alt") || src.split("/").pop() || "Download file";

			// build a clickable download button
			const button = this.pageDocument.createElement("a");
			button.setAttribute("href", src);
			button.setAttribute("download", "");
			button.setAttribute("target", "_blank");
			button.setAttribute("rel", "noopener noreferrer");
			button.className = "internal-embed file-embed mod-generic is-loaded";
			button.style.display = "block";

			// keep the icon immediately left of the title, inline, like Obsidian
			const titleSpan = this.pageDocument.createElement("span");
			titleSpan.className = "file-embed-title";
			...
			// replace the span in place and drop the leftover empty <p> if any
			embed.replaceWith(button);
			const emptyParagraph = button.closest("p")?.parentElement?.querySelector("p:empty");
